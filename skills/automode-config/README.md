...
`autoMode` permission blocks. The skill keeps the four-bucket model
(`allow` / `ask` / `deny` / `hard_deny`) honest, gates every write
behind `claude auto-mode critique`, and writes atomically under per-file
flock with a sha256 hash gate.
...
Claude Code 2.1.136+ supports a four-bucket `autoMode` schema, but
hand-editing the JSON is error-prone:

- It's easy to land a rule in `allow` that should be `ask`, or in
  `deny` instead of the unconditional `hard_deny`.
- The `$defaults` sentinel in `environment` is dropped by accident,
  silently losing the Anthropic-curated trust signals.
...
- The `claude auto-mode critique` invocation is the canonical Path (b)
  validator, but its CLI has version-dependent quirks (no `--settings`
  on older builds, drifting section headers).
...
- **Claude Code 2.1.136+** (the four-bucket model with `hard_deny`).
...
# expect: name: automode-config / metadata.version: 0.4.2 (or higher)
```
...
- **Four-bucket schema awareness.** `hard_deny` round-trips cleanly;
  `--migrate-strategy drop-all` resets all four lists.
...
├── SKILL.md            # Agent-facing entry point (4-bucket model, six phases)
...
├── references/         # Mental model, canonicalization, recovery, verification
...
- `references/mental_model.md` — three files, four buckets, six phases.
- `references/three_files.md` — per-file gotchas and `hard_deny` round-trip.
...
`metadata.version: 0.4.2` (pre-1.0; see `SKILL.md` for the v1.0
roadmap).
